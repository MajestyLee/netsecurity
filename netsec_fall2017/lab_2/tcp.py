...
class passthrough_1(StackingProtocol): #below client 
    def __init__(self):
        super().__init__
    def data_received(self, data):
        print("ps1 received")
...
class passthrough_2(StackingProtocol): #below passthrough_1
    def __init__(self):
        self._deserializer = PacketType.Deserializer()
        super().__init__
        self.status = -1 # set the status
    def data_received(self, data):
        print("ps2 received")
        self._deserializer.update(data)
        for pkt in self._deserializer.nextPackets():
            if isinstance(pkt, lab_1_Packet.Tcppacket):
                if pkt.Type == 1 and self.status == 0:
                    self.status == 1
                    self.higherProtocol().connection_made(StackingTransport(self.transport))
                    print("client connected") #get the syn-ack, connect the higher layer
                    packet = lab_1_Packet.Tcppacket() # ack packet without data
                    packet.Type = 2
                    packet.SequenceNumber = 0
                    packet.Checksum = 0
                    packet.Acknowledgement = pkt.Acknowledgement + 1
                    # print(packet)
                    self.transport.write(packet.__serialize__())
        # self.higherProtocol().data_received(data)
    def connection_made(self, transport):
        ...
        self.transport = transport
        if self.status == -1:
            packet = lab_1_Packet.Tcppacket() # syn packet
            packet.Type = 0
            packet.SequenceNumber = 0
            packet.Checksum = 0
            print(packet)
...
class passthrough_3(StackingProtocol): #the layer below server
    def __init__(self):
        super().__init__
    def data_received(self, data):
        print("ps3 received")
    ...
    def connection_lost(self, exc):
        ...
        self.transport.close()
        self.higherProtocol().transport.close()
class passthrough_4(StackingProtocol): #the layer below passthrough_3
    def __init__(self):
        super().__init__
        self.status = -1
        self._deserializer = PacketType.Deserializer()
        ...
        for pkt in self._deserializer.nextPackets():
            if isinstance(pkt, lab_1_Packet.Tcppacket):
                if pkt.Type == 0:
                    self.status == 0
                    print("receive syn")
                    packet = lab_1_Packet.Tcppacket()#send syn-ack packet
                    packet.Type = 1
                    packet.SequenceNumber = 0
                    packet.Checksum = 0
                    packet.Acknowledgement = 123
                    self.transport.write(packet.__serialize__())
                elif pkt.Type == 2 and self.status == 0:
                    self.status == 1
                    self.higherProtocol().connection_made(StackingTransport(self.transport))
                    print ("server connected") #get ack, higher layer connected
        # self.higherProtocol().data_received(data)
    def connection_made(self, transport):
        print("ps4 con made")
        self.transport = transport
...
if __name__=="__main__":
    ...
    if mode.lower() == "server":
        f = StackingProtocolFactory(lambda: passthrough_3(),lambda: passthrough_4())
